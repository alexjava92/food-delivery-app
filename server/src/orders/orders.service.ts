        ...
        try {
            ...
            if (dto.status) {
                ...
                this.eventsGateway.emitToUser(user.id, 'order-notification', {
                    id: order.id,
                    status: dto.status,
                    message: `Ваш заказ #n${order.id} теперь "${dto.status}"`,
                });
